...
module Apivore
  module RspecMatchers
    ...
    matcher :be_consistent_with_swagger_definitions do |master_swagger_url, current_service|
      ...
      define_method :cleaned_definitions do |definitions, service|
        definitions.inject({}) do |hash, (key, definition_fields)|
          # We ignore definitions that are owned exclusively by the current_service
          next hash if definition_fields['x-services'] == [service]
          ...
          hash[key] = definition_fields.except('x-services')
          hash
        end
      end
