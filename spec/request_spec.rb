...
describe 'Example API', type: :apivore, order: :defined do
  ...
  context 'has valid paths' do
    ...
    it do
      expect(subject).to validate(:post, '/services.json', 204, 'name' => 'hello world')
    end
    ...
    it do
      expect(subject).to validate(:put, '/services/{id}.json', 204, 'id' => 1)
    end

    it do
      expect(subject).to validate(:delete, '/services/{id}.json', 204, 'id' => 1)
    end

    it do
      expect(subject).to validate(:patch, '/services/{id}.json', 204, 'id' => 1)
    end
  end
